...
Let's say we're really into [node-fibers](https://github.com/laverdet/node-fibers), with it's concise coroutines and error handling. We're starting a new module, tests first, so we `npm install mocha mocha-fibers` and write some failing tests.  Next we `npm install [fibrous](https://github.com/goodeggs/fibrous)` to help implement our module.  If we list installed fibers with `npm ls fibers`:
